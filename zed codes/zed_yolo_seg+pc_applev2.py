    ...
    if zed.grab(runtime) == sl.ERROR_CODE.SUCCESS:
        ...
        if boxes is not None:
            ...
            for i, (box, cls_id) in enumerate(zip(boxes.xyxy, boxes.cls)):
                ...
                #pc_filtered_np = np.zeros([x2, y2, 4], dtype=np.float32)  # Initialize filtered point cloud

                pc_filtered_np[y1:y2, x1:x2] = pc_np[y1:y2, x1:x2]

                print(pc_filtered_np.shape)

        # Aggiorna la point cloud filtrata
